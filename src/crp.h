...
// Chinese restaurant process (Pitman-Yor parameters) histogram-based table
// tracking based on the implementation proposed by Blunsom et al. 2009 this
// implementation assumes that the observation likelihoods are either 1 (if
// they are identical to the "parameter" drawn from G_0) or 0. This is fine for
// most NLP applications but violated in PYP mixture models etc.
...
class crp {
 public:
  ...
  crp(double d_strength, double d_beta, double c_shape, double c_rate,
      double d = 0.8, double c = 1.0) :
    num_tables_(),
    num_customers_(),
    discount_(d),
    strength_(c),
  ...
  void check_hyperparameters() {
    ...
    if (strength_ <= -discount_) {
      std::cerr << "Bad strength: " << strength_ << " (discount="
                << discount_ << ")" << std::endl;
      abort();
    }
    ...
    if (discount_) {
      ...
      if (has_discount_prior()) {
        llh_ = Md::log_beta_density(discount_,
                                    discount_prior_strength_,
                                    discount_prior_beta_);
      }
      if (has_strength_prior()) {
        llh_ += Md::log_gamma_density(
            strength_ + discount_,
            strength_prior_shape_,
            strength_prior_rate_);
      }
      if (num_tables_ > 0) {
      ...
      if (has_strength_prior()) {
        llh_ += Md::log_gamma_density(
            strength_, strength_prior_shape_, strength_prior_rate_);
      }
      if (num_tables_ > 0) {
        llh_ = log_likelihood(discount_, strength_);
      }
  ...
  int decrement(const Dish& dish, Engine& eng, double* logq = nullptr) {
    ...
    if (loc.num_customers() == 1) {
      ...
      update_llh_remove_customer_from_table_seating(
          selected_table_postcount + 1);
      --num_customers_;
      if (delta) {
        --num_tables_;
      }
  ...
  F prob(const Dish& dish, const F& p0) const {
    ...
    if (it == dish_locs_.end()) {
      ...
      return (F(it->second.num_customers() -
                discount_ * it->second.num_tables()) + r * p0) /
  ...
  double log_likelihood(const double& discount, const double& strength) const {
    ...
    if (has_discount_prior() && discount) {
      lp = Md::log_beta_density(
          discount, discount_prior_strength_, discount_prior_beta_);
    }
    if (has_strength_prior()) {
      lp += Md::log_gamma_density(
          strength + discount, strength_prior_shape_, strength_prior_rate_);
    }
    assert(lp <= 0.0);
    if (num_customers_) {  // if restaurant is not empty
      if (discount > 0.0) {  // two parameter case: discount > 0
        ...
        lp += - lgamma(strength + num_customers_)
             + num_tables_ * log(discount) +
             lgamma(strength / discount + num_tables_);
        // above line implies
        ...
        //    lp += log(strength + N - 1)
        //    [because \Gamma(s+N) = (s+N-1)\Gamma(s+N-1)]
        ...
        lp += lgamma(strength) + num_tables_ * log(strength) -
            lgamma(strength + num_customers_);
        assert(std::isfinite(lp));
        for (auto& dish_loc : dish_locs_)
          for (auto& bin : dish_loc.second.h[0])
                                ...
                                const unsigned niterations = 10) 
  {
    ...
    for (unsigned iter = 0; iter < nloop; ++iter) {
      if (has_strength_prior()) {
        s = slice_sampler1d([this, d](double prop_s) {
          return this->log_likelihood(d, prop_s); },
      ...
      if (has_discount_prior()) {
        ...
        d = slice_sampler1d([this, s](double prop_d) {
          return this->log_likelihood(prop_d, s); },
          d, eng, min_discount,
  ...
  void print(std::ostream* out) const {
    (*out) << "PYP(d=" << discount_ << ",c=" << strength_
           << ") customers=" << num_customers_ << std::endl;
    for (auto& dish_loc : dish_locs_)
      (*out) << dish_loc.first << " : " << dish_loc.second << std::endl;
  }

  typedef typename
      std::unordered_map<Dish, crp_table_manager<1>, DishHash>::const_iterator
      const_iterator;
